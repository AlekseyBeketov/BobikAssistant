...
//using System.Speech.Synthesis; Только для винды
using DotNetEnv;
using System.Timers;
...
namespace BobikAssistant
{
    public partial class MainPage : ContentPage, INotifyPropertyChanged
    {
        ...
        private static readonly string VoskModelPath = Path.Combine(AppFileSystem.VoskModelsPath, "vosk-model-small-ru-0.22");
        ...
        public MainPage()
        {
            ...
            string folderPath = AppFileSystem.VoiceRecordsPath;
        ...
        private void SpeakText(string text)
        {
            ...
            {
                Rate = AVSpeechUtterance.DefaultSpeechRate, // Настроить скорость
                Voice = AVSpeechSynthesisVoice.FromLanguage("ru-RU") // Устанавливаем русский язык
            };

            synthesizer.SpeakUtterance(speechUtterance);
            #else
            Console.WriteLine("Необходимо использовать другие библиотеки");

            #endif
        }

        // Метод для остановки текущего воспроизведения речи
        ...
        private async Task SendToOllamaAsync(string text)
        {
			...
			using (HttpClient client = new HttpClient())
			{
				...
				var requestBody = new
				{
					...
					stream = false  // Пытаемся сделать потоковый режим
				};
		...
		public class Message : INotifyPropertyChanged
		{
			...
			private const string FilterPhrase = "Отвечай как голосовой ассистент, дружелюбно и по делу, но строго не более 650 символов!";
